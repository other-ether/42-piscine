...
/*   Updated: 2026/06/07 16:18:08 by guoliu           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	try_min_max_right(int values[], int solution[4][4])
{
	...
	while (i < 16)
	{
		if (values[i] == 1)
		{
			if (solution[3][i - 12] == 0)
				solution[3][i - 12] = 4;
			else if (solution[3][i - 12] != 4)
				return (0);
		}
		else if (values[i] == 4)
		{
